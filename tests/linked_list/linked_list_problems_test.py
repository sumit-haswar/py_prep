...
class LinkedListProblemsTests(unittest.TestCase):
    ...
    def test_front_back_split(self):
        ...
        # assert front back
        self.assertListEqual(get_list_from_linked_list(front), [2, 4, 12])
        self.assertListEqual(get_list_from_linked_list(back), [15, 17, 32])

        list = create_list([2, 4, 12, 15, 17, 32, 34])
        front, back = front_back_split(list)

        # assert front back
        self.assertListEqual(get_list_from_linked_list(front), [2, 4, 12, 15])
        self.assertListEqual(get_list_from_linked_list(back), [17, 32, 34])

    def test_remove_duplicates(self):
        result = remove_duplicates(create_list([2, 3, 4, 4, 4, 5, 6, 7, 7]))
        self.assertListEqual(get_list_from_linked_list(result), [2, 3, 4, 5, 6, 7])
    ...
    def test_move_node(self):
        ...
        self.assertListEqual(get_list_from_linked_list(dest), [1, 5, 6, 7])
        self.assertListEqual(get_list_from_linked_list(source), [2, 3, 4])

    def test_alternating_split(self):

        even, odd = alternating_split(create_list([11, 22, 33, 44, 55, 66]))

        self.assertListEqual(get_list_from_linked_list(even), [11, 33, 55])
        self.assertListEqual(get_list_from_linked_list(odd), [22, 44, 66])

    def test_shuffle_merge(self):
        result = shuffle_merge(create_list([1, 2, 3]), create_list([7, 13, 1]))

        self.assertListEqual(get_list_from_linked_list(result), [1, 7, 2, 13, 3, 1])

    def test_sorted_merge(self):
        result = sorted_merge(create_list([11, 13, 17, 34, 90, 123]), create_list([3, 14, 99, 111, 150]))
        self.assertListEqual(
            get_list_from_linked_list(result),
    ...
    def test_merge_sort(self):
        ...
        random_list.sort()
        self.assertListEqual(get_list_from_linked_list(result), random_list)
    ...
    def test_sorted_intersect(self):
        ...
        self.assertListEqual(get_list_from_linked_list(result), [2, 3, 5, 8, 9, 19, 28, 90])

    def test_reverse(self):

        result = reverse(create_list([3, 4, 5, 6, 7, 8]))
        self.assertListEqual(get_list_from_linked_list(result), [8, 7, 6, 5, 4, 3])

    def test_reverse_recursive(self):
        result = recursive_reverse(create_list([8, 22, 6, 12, 234, 56, 20]))
        self.assertListEqual(get_list_from_linked_list(result), [20, 56, 234, 12, 6, 22, 8])
